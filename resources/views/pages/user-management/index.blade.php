    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
        ...
        <div class="flex flex-col gap-3 md:hidden">
            @forelse($users as $user)
                @php $lingkupJabatan = $user->userRoles->first()?->organization?->name ?? '—'; @endphp
                <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-white/[0.03]">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span class="shrink-0 inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $user->is_active ? 'bg-success-100 text-success-700' : 'bg-error-100 text-error-700' }}">
                    ...
                    <p class="mt-1.5 truncate text-xs text-gray-500 dark:text-gray-400">
                        <span class="font-medium text-gray-600 dark:text-gray-300">Lingkup:</span> {{ $lingkupJabatan }}
                    </p>
                    <div class="mt-3 flex items-center gap-3 border-t border-gray-200 pt-3 dark:border-gray-700">
                        <a href="{{ route('app.users.edit', $user->id) }}" class="text-sm text-brand-600 hover:text-brand-700 font-medium">Edit</a>
                        @if(in_array($user->id, $deletableUserIds ?? [], true))
                            <button type="button" class="text-sm text-error-600 hover:text-error-700 font-medium"
            ...
            @empty
                <p class="py-6 text-center text-sm text-gray-500">Belum ada user.</p>
            @endforelse
        </div>

        {{-- Desktop: tabel --}}
        <div class="hidden md:block overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800">
            <div class="overflow-x-auto">
                <table class="min-w-full">
                    <thead class="bg-gray-50 dark:bg-gray-800/70">
                        <tr>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[20%]">Nama</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[22%]">Email</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[14%]">Role</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[24%]">Lingkup Jabatan</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[10%]">Status</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-500 w-[10%]">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse($users as $user)
                            ...
                            <tr class="bg-white transition-colors hover:bg-gray-50 dark:bg-transparent dark:hover:bg-white/5">
                                <td class="px-3 py-2 text-sm font-medium text-gray-800 dark:text-white/90 max-w-0">
                                    <div class="truncate" title="{{ $user->name }}">{{ $user->name }}</div>
                                </td>
                                <td class="px-3 py-2 text-sm text-gray-600 dark:text-gray-300 max-w-0">
                                ...
                                <td class="px-3 py-2 text-sm text-gray-600 dark:text-gray-300 max-w-0">
                                    <div class="truncate" title="{{ $lingkupJabatan }}">{{ $lingkupJabatan }}</div>
                                </td>
                                ...
                                <td class="px-3 py-2 text-sm whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @if(in_array($user->id, $deletableUserIds ?? [], true))
                                            <button type="button" class="text-error-600 hover:text-error-700"
                                                data-action="{{ route('app.users.destroy', $user->id) }}"
                                                data-name="{{ $user->name }}"
                                                @click="deleteAction = $el.dataset.action; deleteUserName = $el.dataset.name; $dispatch('open-user-delete-modal')">
                                                Hapus
