...
@php
    ...
    $iconSize = $iconSizeClasses[$size] ?? $iconSizeClasses['medium'];
    $statusSizeClass = $statusSizeClasses[$size] ?? $statusSizeClasses['medium'];
    $statusColorClass = $statusColorClasses[$status] ?? '';
@endphp

<div class="relative rounded-full {{ $sizeClass }} flex items-center justify-center bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300">
    <svg data-feather="user" width="{{ $iconSize }}" height="{{ $iconSize }}"></svg>
