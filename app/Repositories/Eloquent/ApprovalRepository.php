...
class ApprovalRepository implements ApprovalRepositoryInterface
{
    ...
    public function paginatePendingForUser(User $user, array $filters = [], int $perPage = 10, string $pageName = 'page'): LengthAwarePaginator
    {
        $query = DocumentApproval::query()
            ...
            ->whereHas('document.workflowInstances', fn ($q) => $q->whereNull('finished_at'))
            ->whereHas('document.workflowInstances', function ($q) {
                $q->whereColumn('document_workflow_instances.current_step_order', 'document_approvals.step_order');
            })
            ->whereHas('workflowStep', function ($q) use ($user) {
                $q->whereIn('role_id', $user->userRoles()->pluck('role_id')->all());
            });
